      ...
      let emissionTask = Task {
        do {
          for try await event in baseStream {
            let isTerminalSuccess = if case .turnCompleted = event { true } else { false }
            ...
            if isTerminalSuccess {
              // Once terminal success has been delivered, the turn is complete.
              // Do not let cancellation during executor teardown replace it.
              continuation.finish()
              return
            }
