...
struct CodexTests {
  ...
  func runStreamedFinishesSuccessfullyAfterTerminalSuccessCancellation() async throws {
    ...
    executor.postTerminalSuccessDelayNanoseconds = 50_000_000
    let handoff = StreamHandoff<StreamedTurn>()
    let terminalEventReceived = OneShotSignal()
    let cancellationApplied = OneShotSignal()
    ...
    let stream = await handoff.wait()
    let consumerTask = Task {
      var collected: [ThreadEvent] = []
      for try await event in stream {
        collected.append(event)
        if case .turnCompleted = event {
          await terminalEventReceived.signal()
          await cancellationApplied.wait()
        }
      }
      return collected
    }

    await terminalEventReceived.wait()
    originatingTask.cancel()
    await cancellationApplied.signal()
